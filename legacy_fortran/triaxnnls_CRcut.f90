...
! triaxnnls_CRcut.f90, 2019/11/18 updated from triaxnnls.f90 !
! There was problem with triaxnnls.f90 in fitting fast rotating galaxies!
! To solve the problem, we cut some orbits with |V - V_obs|> 3sigma_obs !
! Which should have no noticable effects in fitting slow rotating galaxies!
! But solve the CR orbits problem in fitting fast rotating galaxies !
! See Zhu+2018 MNRAS 2018 473 3000 for detail !
        ...
        do i = 1, 2 ! loop over orbit libraries
            ...
            do j = 1, norb(i) ! loop over orbits
                ...
                do k = 1, -1 + (i - 1)*2, -2
                    ...
                    do l = 1, nconstr ! loop over aperture
                        ...
                        if (sum(velhist(:, l)) > 0.0_dp) then
                            ...
                            ! should still divide by dvhist.
                                  ...
                                  !!!!!!!!!!!!!!!!!!!!!!!!!!
                            ! CRcut
                            ! calculate the mean velocity of the particles from orbit 'orboffset' passing aperture 'l'
                            ! vm_ol
                            vm_ol = 0
                            norm_veltmp = 0
                            ...
                            !For the orbit 'orboffset', comparing its vm_ol with each aperture l it passes !
                            !The orbit is considered to be bad for aperture l if it satisfy the following conditions  !
                    ...
                    ! Loop over apertures finidhed !

                    ! If orbit 'orboffset' is bad for > 1 apertures, loop over apertures again!
                    ! We cut the orbits at these apertures it is bad for!
...
! Comparing to triaxnnls.f90, we add one column to store lcut
! lcut = 0 : the orbit is normal, lcut > 0: the orbit is excluded
                ...
                do k = 1, -1 + (i - 1)*2, -2
                    ...
                    if (orbweight(orboffset) > 0.0_dp .and. orb_qualify == 1) then
                        ...
                        quad_store(2, :, :, :) = quad_store(2, :, :, :) &
                                                 + orbweight(orboffset)*quad_light(2, :, :, :)*quad_light(1, :, :, :)
                        quad_store(3, :, :, :) = quad_store(3, :, :, :) &
                                                 + orbweight(orboffset)*quad_light(3, :, :, :)*quad_light(1, :, :, :)
                        quad_store(4, :, :, :) = quad_store(4, :, :, :) &
                                                 + orbweight(orboffset)*quad_light(4, :, :, :)*quad_light(1, :, :, :)
                        quad_store(5, :, :, :) = quad_store(5, :, :, :) &
                                                 + orbweight(orboffset)*quad_light(5, :, :, :)*quad_light(1, :, :, :)*k
                        quad_store(6, :, :, :) = quad_store(6, :, :, :) &
                                                 + orbweight(orboffset)*quad_light(6, :, :, :)*quad_light(1, :, :, :)*k
                        quad_store(7, :, :, :) = quad_store(7, :, :, :) &
                                                 + orbweight(orboffset)*quad_light(7, :, :, :)*quad_light(1, :, :, :)*k
                        quad_store(8, :, :, :) = quad_store(8, :, :, :) &
                                                 + orbweight(orboffset)*quad_light(8, :, :, :)*quad_light(1, :, :, :)
                        quad_store(9, :, :, :) = quad_store(9, :, :, :) &
                                                 + orbweight(orboffset)*quad_light(9, :, :, :)*quad_light(1, :, :, :)
                        quad_store(10, :, :, :) = quad_store(10, :, :, :) &
                                                  + orbweight(orboffset)*quad_light(10, :, :, :)*quad_light(1, :, :, :)
                        quad_store(11, :, :, :) = quad_store(11, :, :, :) &
                                                  + orbweight(orboffset)*quad_light(11, :, :, :)*quad_light(1, :, :, :)
                        quad_store(12, :, :, :) = quad_store(12, :, :, :) &
                                                  + orbweight(orboffset)*quad_light(12, :, :, :)*quad_light(1, :, :, :)
                        quad_store(13, :, :, :) = quad_store(13, :, :, :) &
                                                  + orbweight(orboffset)*quad_light(13, :, :, :)*quad_light(1, :, :, :)
                        quad_store(14, :, :, :) = quad_store(14, :, :, :) &
                                                  + orbweight(orboffset)*quad_light(14, :, :, :)
                        quad_store(15, :, :, :) = quad_store(15, :, :, :) &
                                                  + orbweight(orboffset)*quad_light(15, :, :, :)
                        quad_store(16, :, :, :) = quad_store(16, :, :, :) &
                                                  + orbweight(orboffset)*quad_light(16, :, :, :)
